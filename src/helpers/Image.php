...
class Image
{
    ...
    {
        return $this->_validation . $this->_image->saveXML($this->_svgElement);
    }
    ...
    private function getFile(): void
    {
        ...
        if ($this->_image->load($name)) {
            $this->_isCustomFile = true;
        } elseif (!$this->_image->load($fileName)) {
            $this->_image->load($this->_defaults->fallbackIcon);
        }

        $this->_svgElement = $this->_image->getElementsByTagName('svg')->item(0);
    }
    ...
    {
        $svgImage = new Svg($this->_svgElement);
        [$svgWidth, $svgHeight] = $svgImage->getSize();
    ...
    private function setAttributes(): void
    {
        if ($title = $this->_options->removeValue('title')) {
            $titleElement = $this->_image->createElement('title', $title);
